...
const Dashboard = () => {
  ...
  const sendMessage = async () => {
    if (!message.trim() || !selectedUser || selectedUser._id === currentUser) {
      return; // Prevent sending message to self
    }
      ...
      <div className="w-2/3 p-4 flex flex-col">
        ...
        <div className="mt-4 flex items-center">
          <input
            ...
            placeholder="Type a message..."
            className="flex-1 p-2 border rounded bg-gray-700 text-white"
            disabled={!selectedUser || selectedUser._id === currentUser} // Disable input if selectedUser is currentUser
          />
          <button
            onClick={sendMessage}
            className="ml-2 bg-purple-600 text-white px-4 py-2 rounded"
            disabled={!selectedUser || selectedUser._id === currentUser} // Disable button if selectedUser is currentUser
          >
            Send
          </button>
        </div>
